...
echo -e "IP_Address","IsPublic","IPversion","IsWhitelisted","Score","CountryCode","CountryName","UsageType","ISP","Domain","Hostnames","TotalReports","NumberOfDistictUsers","LastReportedAt","ReportedAt","Comment","Categories","ReportedID","ReporterCountryCode","ReporterCountryName\n"  >> IP_Abuseip_Stats.csv
while read line; do
    ipcount=$(( ipcount+1 ))
    Vfinal=()
    echo "[A$ipcount] --------------------------------"$line"--------------------------------------"
...
if (( $s != 0 ))
    then
	    ...
	    ip=$(jq -r '.data."ipAddress"' temp.json)        
            ip="${ip//,/}"
	    
	    ispub=$(jq -r '.data."isPublic"' temp.json)        
            ispub="${ispub//,/}"
	    
	    ipv=$(jq -r '.data."ipVersion"' temp.json)        
            ipv="${ipv//,/}"
	    
	    iswhi=$(jq -r '.data."isWhitelisted"' temp.json)        
            iswhi="${iswhi//,/}"
	    
	    score=$(jq -r '.data."abuseConfidenceScore"' temp.json)        
            score="${score//,/}"
	    
	    cc=$(jq -r '.data."countryCode"' temp.json)        
            cc="${cc//,/}"
	    
	    cn=$(jq -r '.data."countryName"' temp.json)        
            cn="${cn//,/}"
	    
	    ut=$(jq -r '.data."usageType"' temp.json)        
            ut="${ut//,/}"
	    
	    isp=$(jq -r '.data."isp"' temp.json)        
            isp="${isp//,/}"
            
	    domain=$(jq -r '.data."domain"' temp.json)        
	    domain="${domain//,/}"
	    
	    hostnames=$(jq -r '.data.hostnames[0] | keys[]' temp.json)        
            hostnames="${hostnames//,/}"
	    
	    totalr=$(jq -r '.data."totalReports"' temp.json)        
            totalr="${totalr//,/}"
	    
	    ndu=$(jq -r '.data."numDistinctUsers"' temp.json)        
            ndu="${ndu//,/}"
	    
	    lra=$(jq -r '.data."lastReportedAt"' temp.json)        
            lra="${lra//,/}"
	    
	    ra=$(jq -r '.data.reports[0]."reportedAt"' temp.json)        
            ra="${ra//,/}"
	    
	    comment=$(jq -r '.data.reports[0]."comment"' temp.json)        
            comment="${comment//,/}"
	    
	    cat=$(jq -r '.data.reports[0].categories[0] | keys[]' temp.json)        
            cat="${cat//,/}"
	    
	    ri=$(jq -r '.data.reports[0]."reportedId"' temp.json)        
            ri="${ri//,/}"
	    
	    rcc=$(jq -r '.data.reports[0]."reporterCountryCode"' temp.json)        
            rcc="${rcc//,/}"
	    
	    rcn=$(jq -r '.data.reports[0]."reporterCountryName"' temp.json)        
            rcn="${rcn//,/}"

	    echo -e "$ip,$ispub,$ipv,$iswhi,$score,$cc,$cn,$ut,$isp,$domain,$hostnames,$totalr,$ndu,$lra,$ra,$comment,$cat,$ri,$rcc,$rcn\n"  >> IP_Abuseip_Stats.csv
